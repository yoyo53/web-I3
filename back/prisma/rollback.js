...
prisma.$queryRaw`SELECT migration_name FROM _prisma_migrations WHERE rolled_back_at IS NULL ORDER BY finished_at DESC LIMIT 1`
    .then((lastMigration) => {
        ...
        if (!fs.existsSync(path.join(migrationFolder, 'down.sql'))) {
            console.error(`\x1b[31mCould not find the rollback file at prisma/migrations/${migrationName}/down.sql.\x1b[0m`);
            process.exit(1);
        }


        rl.question(`✔ \x1b[1mAre you sure you want to rollback migration \`${migrationName}\`? (y/N): \x1b[31mData may be lost\x1b[0m. … `, async (confirmation) => {
            console.log();
            ...
            try {
                ...
                execSync(`npx prisma db execute --file "${path.join(migrationFolder, 'down.sql')}" --schema prisma/schema.prisma`, { stdio: 'pipe' });
